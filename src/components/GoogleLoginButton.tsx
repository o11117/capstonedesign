...
const GoogleLoginButton = () => {
  ...
  const extractCodeAndSendToBackend = async () => {
    ...
    try {
      const res = await axios.post(
        'https://port-0-planit-mcmt59q6ef387a77.sel5.cloudtype.app/api/auth/google',
        { code },
        {
  ...
  return (
    <button onClick={handleLogin} className={`${styles.googleLoginButton} ${styles.socialButton}`}>
