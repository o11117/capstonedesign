  ...
  persist(
    (set, get) => ({
      ...
      addCourse: async (title, startDate, endDate, userId) => {
        ...
        try {
          const res = await fetch('https://port-0-planit-mcmt59q6ef387a77.sel5.cloudtype.app/api/schedules', {
            method: 'POST',
            headers: {
      ...
      removeCourse: async (courseId) => {
        try {
          await fetch(`https://port-0-planit-mcmt59q6ef387a77.sel5.cloudtype.app/api/schedules/${courseId}`, {
            method: 'DELETE',
          })
        } catch (err) {
